...
import { getBudgetVsExpense, getFinancialOverview, getRevenueTrends } from "../controllers/report"
...
export const financialOverview = async (req: Request, res: Response) => {
  try {
    const data = await getFinancialOverview();

    res.status(200).json({
      msg: "Financial report fetched successfully",
      data
    })
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : "Unknown error";
    ...
    res.status(500).json({
      msg: "Failed to fetch financial report",
      error: errorMessage
    })
  }
}